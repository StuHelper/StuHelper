...
// GetNLS returns NLS properties using one fixed SELECT. Upstream v2.9.0 uses a
// PL/SQL block for this fallback, which is outside StuHelper's
// authentication-and-SELECT-only Oracle policy.
func (conn *Connection) GetNLS() (*NLSData, error) {
	const query = `SELECT
		MAX(CASE WHEN PARAMETER='NLS_CALENDAR' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_COMP' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_LENGTH_SEMANTICS' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_NCHAR_CONV_EXCP' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_DATE_LANGUAGE' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_SORT' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_CURRENCY' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_DATE_FORMAT' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_ISO_CURRENCY' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_NUMERIC_CHARACTERS' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_DUAL_CURRENCY' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_TIMESTAMP_FORMAT' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_TIMESTAMP_TZ_FORMAT' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_LANGUAGE' THEN VALUE END),
		MAX(CASE WHEN PARAMETER='NLS_TERRITORY' THEN VALUE END)
	FROM SYS.NLS_SESSION_PARAMETERS`
	err := conn.QueryRowContext(context.Background(), query, nil).Scan(
		&conn.NLSData.Calender,
		&conn.NLSData.Comp,
		&conn.NLSData.LengthSemantics,
		&conn.NLSData.NCharConvExcep,
		&conn.NLSData.DateLang,
		&conn.NLSData.Sort,
		&conn.NLSData.Currency,
		&conn.NLSData.DateFormat,
		&conn.NLSData.IsoCurrency,
		&conn.NLSData.NumericChars,
		&conn.NLSData.DualCurrency,
		&conn.NLSData.Timestamp,
		&conn.NLSData.TimestampTZ,
		&conn.NLSData.Language,
		&conn.NLSData.Territory,
	)
	if err != nil {
		return nil, err
	}
...
func (conn *Connection) Begin() (driver.Tx, error) {
	return nil, errStuHelperSelectOnly
}

func (conn *Connection) BeginTx(context.Context, driver.TxOptions) (driver.Tx, error) {
	return nil, errStuHelperSelectOnly
}
...
func NewConnection(databaseUrl string, config *configurations.ConnectionConfig) (*Connection, error) {
	...
	*temp = *config
	return &Connection{
		...
		// StuHelper never sets Oracle auto-commit flags. This package rejects
		// transactions and Exec calls, so SELECT cursors need no commit lifecycle.
		autoCommit: false,
		cusTyp:     map[string]customType{},
		maxLen: struct {
			varchar   int
