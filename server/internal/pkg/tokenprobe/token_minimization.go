...
func ProbeApplicationTokenFieldsMinimized(
	tokenFormat string,
	tokenFields []string,
) (TokenMinimizationProbeResult, error) {
	if tokenFormat != "JWT-Custom" {
	...
	fields, err := NormalizeExplicitTokenFields(tokenFields)
	if err != nil {
...
// NormalizeExplicitTokenFields normalizes an explicitly configured Casdoor token field list.
...
func normalizeList(values []string) ([]string, error) {
	...
	for _, raw := range values {
		...
		if value == "" {
			return nil, errors.New("casdoor: blank token field is forbidden")
		}
