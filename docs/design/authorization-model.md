...
当前运行时已通过 `server/internal/platform/authorization.RoleScopeResolver` 为 `school_admin` 补全 scope：先把 Casdoor subject 映射到内部 `users.id`，再从 OpenFGA 查询 `school#effective_admin`。OpenFGA tuple 中的 `user:<id>` 必须使用内部 `users.id`，不能使用 Casdoor subject。
