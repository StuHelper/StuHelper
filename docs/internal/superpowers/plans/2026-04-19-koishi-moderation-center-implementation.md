...
status: snapshot
authoritative-source: this file (future implementation snapshot)
last-verified: 2026-04-19
---

# Koishi Moderation Center Implementation Plan

> 现状说明：本文是未来群管中心的实现快照，不代表当前仓库已经全部落地这些能力。
>
> 当前已实现：QQ 绑定命令、入群禁言/解禁/超时踢出、关键词处理、复读检测、撤回留痕、举报命令、骰子/抽禁言、完整的 `群审*` 文本命令、`stuhelper-console` 自定义控制台、举报面板、群模板/群绑定面板、SQLite 留痕与自动化测试。
>
> 当前未实现：更重型的运营工作流，例如更细粒度报表、历史版本和更复杂的处置编排。
...
- [ ] 写失败测试，覆盖警告阈值表达式、复读命中和关键词动作决策。
- [ ] 运行 `cd bots/koishi && corepack yarn test:unit --test-name-pattern \"moderation-core\"`，确认新测试先失败。
- [ ] 实现群管域常量、模型注册、存储访问和规则引擎最小闭环。
- [ ] 复跑对应单测，确认转绿。
...
- [ ] 扩展核心配置，增加 `moderation`、`fun`、`console`、`ai` 等分组。
- [ ] 定义事件日志、复核队列、成员处罚、角色权限、关键词和娱乐状态等共享类型。
- [ ] 确保 `@stuhelper/koishi-shared` 只保留平台 API、配置工厂和跨插件复用的轻量类型。
...
- [ ] 先写失败测试，覆盖入群准入、关键词警告、复读处罚、撤回留痕和举报入队。
- [ ] 在插件中接入 `database`、`commands` 和 `moderation-core`，拆分事件监听与命令注册。
- [ ] 让消息账本、告警累计、自动禁言、人工复核队列和娱乐命令都写入 SQLite。
- [ ] 复跑插件测试，确认核心链路通过。
...
- [ ] 先写失败测试，覆盖成员列表、批量禁言、警告查询、踢人申请和复核队列查看。
- [ ] 使用 Koishi 通用命令体系注册分层管理员命令，并为高风险动作保留“提交复核”而非直接执行。
- [ ] 为每条命令设置明确 `authority`，避免依赖自定义裸判断。
- [ ] 复跑命令测试。
...
- [ ] 先写服务层失败测试，覆盖概览数据聚合和高风险动作监听。
- [ ] 使用 `ctx.inject(['console'], ...)`、`ctx.console.addEntry()` 和 `DataService` 暴露控制台数据。
- [ ] 使用自定义页面实现三栏工作台最小版本，包含总览、队列、日志和详情面板。
- [ ] 为控制台操作监听设置显式 `authority`，并让所有高风险动作进入人工复核队列。
...
- [ ] 将 `stuhelper-console` 纳入 `stuhelper-core` 装配。
- [ ] 启用 Koishi Console 所需认证与页面入口，固定端口仍为 `5140`。
- [ ] 更新 README，说明插件职责、数据边界和验证命令。
- [ ] 如实现与原设计不一致，以代码为准同步修正文档。
...
- [ ] 运行 `cd bots/koishi && corepack yarn build`
- [ ] 运行 `cd bots/koishi && corepack yarn test`
- [ ] 运行 `cd bots/koishi && NODE_ENV=development timeout 30s corepack yarn dev`
- [ ] 确认控制台插件加载、端口固定在 `5140`、现有绑定/群审能力未回归。
