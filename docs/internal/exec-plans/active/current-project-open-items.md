...
本文件只记录当前仍然活跃的工作项。历史身份入口方案、历史公网 smoke 证据和已废弃迁移路径不再作为当前待办或验收来源。
...
| 任务 | 当前状态 | 完成标准 |
|------|----------|----------|
| 清理独立身份入口 | 独立 issuer 代码、bootstrap、smoke、prod parity 和长期 404 契约已删除 | 全仓库不再包含独立身份域；不再有旧迁移指南、旧 ingress diagnostic、旧 disabled-host smoke 或长期 404 契约 |
...
| 外部学籍数据源模块 | 已新增外部数据源模块并支持 Oracle 只读查询配置；admission readiness 可识别 `external_oracle` 与本地 fallback；`external-student-source-smoke.sh` 可验证真实 Oracle 连通、可读行和可选抽样匹配；本地真实只读 smoke 已确认 `USR_JWBIZ.T_XS_JBXX` 存在至少一条 `XH/XM` 非空记录 | 在生产 secret backend 中配置外部学籍源和可选抽样学号/姓名后，用 `external-student-source-smoke.sh` 留档；evidence 不含原始学号、姓名或密码 |
