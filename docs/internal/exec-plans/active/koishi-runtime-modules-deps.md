...
- `help`, `dice`, `banme`, `config`, and `status` have been migrated to native `RuntimeModule`
  implementations in P4b-1 through P4b-5; the remaining rows still use `BaseModuleAdapter`.
- `log`, `keyword`, `repeat`, `ai`, `event`, and `subscription` install listeners, middleware, or
  timers. P4b native modules must preserve their dispose behavior instead of relying on silent
  fallback cleanup.
