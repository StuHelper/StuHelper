...
- `help`, `dice`, `banme`, `config`, `status`, `event`, `antirepeat`, `manage-message`,
  `getauth`, `manage-cross-group`, `auth`, and `repeat` have been migrated to native
  `RuntimeModule` implementations in P4b-1 through P4b-12; the remaining rows still use
  `BaseModuleAdapter`.
