...
- `help`, `dice`, `banme`, `config`, `status`, `event`, `antirepeat`, `manage-message`,
  `getauth`, `manage-cross-group`, and `auth` have been migrated to native `RuntimeModule`
  implementations in P4b-1 through P4b-11; the remaining rows still use `BaseModuleAdapter`.
