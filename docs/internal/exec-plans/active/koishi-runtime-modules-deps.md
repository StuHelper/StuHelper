...
- `help`, `dice`, `banme`, `config`, `status`, `event`, `antirepeat`, `manage-message`,
  ...
  `manage-member`, `manage-order`, `keyword`, and `ai` have been migrated to native `RuntimeModule` implementations in P4b-1 through P4b-20; the remaining rows still use
  `BaseModuleAdapter`.
