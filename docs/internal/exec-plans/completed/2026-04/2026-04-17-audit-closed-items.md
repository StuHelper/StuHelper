...
说明：
- 这些文件是原始并行审计输入，内容与 `merged-audit`、`completed` 长期重复。
- 审计主清单已于 2026-04-19 全量核销并归档到 `/Users/zxy/Code/StuHelper/docs/exec-plans/archived/merged-audit-2026-04-16.md`。
...
### ✅ [FIXED] 关键覆盖率门禁已达当前基线
（来源: doc1 GO-H6@81）
已修复：`cd server && bash scripts/check-coverage-threshold.sh` 已于 2026-04-19 复核通过；最新实测为 `internal/modules/auth` 80.0%、`internal/modules/course` 81.3%、`internal/modules/course/review` 79.3%、`internal/pkg/middleware` 82.0%、`internal/pkg/oidc` 83.7%、`internal/pkg/fga` 82.7%。active 审计文档中“关键覆盖率仍未全部达到目标”的遗留条目已移除。
...
已处置：`docs/exec-plans/archived/iam-architecture-migration.md` 已归档；该计划不再占用 active 审计清单。
