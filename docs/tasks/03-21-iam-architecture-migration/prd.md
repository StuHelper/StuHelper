...
**Decision**:
- Zitadel 管理 5 个粗角色：super_admin / school_admin / moderator / verified_student / user
...
### D4: 认证分层——过程和真相源在应用，Zitadel 只存粗角色
...
- Zitadel 只存粗角色（verified_student），作为 Token claim 的粗筛依据
...
- [ ] Zitadel 自托管部署（Docker Compose + PostgreSQL）
- [ ] 创建 Organization 结构（stuhelper-platform + buaa）
- [ ] 创建 Project hangxiaoban + Application（web / admin / mobile）
- [ ] 定义 Project Roles（super_admin / school_admin / moderator / verified_student / user）
- [ ] 配置 SMTP（邮件验证）
- [ ] 配置通用 HTTP SMS（腾讯云短信转发）
- [ ] 自定义登录 UI（Vue，支持手机号验证码登录）
- [ ] Google Fonts 替换为自托管

### SSO 迁移

- [ ] 替换 SSO 客户端：删除 `pkg/sso/` 整个包，用标准 Go OIDC 库
- [ ] 重写 JWT 验证：标准 OIDC 验证，不再需要 Casdoor 特定 claims struct
- [ ] 重写 login/callback/refresh/logout handler
- [ ] 重写 Token cookie 管理
- [ ] 重写 auth 中间件（Cookie + Bearer，从 Token 读角色和元数据）
- [ ] 用户数据迁移脚本（Casdoor 用户 → Zitadel 用户）

### 权限简化

- [ ] 定义角色→能力静态映射（Go 常量 map）
- [ ] 重写中间件：Token 读角色 → 内存展开能力 → 注入 context
- [ ] 删除 6 张 RBAC 表（roles / permissions / role_permissions / user_roles / user_groups / user_group_members / user_group_permissions / user_permissions）
- [ ] 删除 RBAC module 的权限计算相关代码
- [ ] RBAC 管理 API 迁移：角色管理 → Zitadel Management API

### 授权层（OpenFGA）

- [ ] OpenFGA 自托管部署
- [ ] 编写授权模型（ecosystem / school / course / review / report / user_profile）
- [ ] 编写 Go 客户端封装（Check / Write / Delete）
- [ ] 资源创建时自动写入关系 tuple
- [ ] 关键操作前调 OpenFGA Check
- [ ] 区分志愿者/管理员权限边界（删帖 vs 看实名信息）

### 认证流程

- [ ] 实名认证：应用触发 → 腾讯云核验 → 结果存应用 DB（user_identities，真相源）
- [ ] 学生认证：LDAP/手动 → 审核 → 结果存应用 DB（user_profiles，真相源）→ 异步同步 Zitadel 粗角色（verified_student）
- [ ] 认证撤销：应用 DB 立即更新 → 异步移除 Zitadel 角色
- [ ] 前端适配：粗筛从 Token 角色读，精确判断调应用接口查实时状态

### 文档更新

- [ ] 重写 `docs/architecture/ecosystem-identity-and-authorization.md`
- [ ] 重写 `docs/modules/auth/` 全部文档
- [ ] 重写 `docs/modules/rbac/README.md`
- [ ] 重写 `docs/modules/policy/` 全部文档
- [ ] 标记 `docs/casdoor/` 为废弃或删除
- [ ] 更新 `docs/architecture/README.md` 索引
- [ ] 更新 `.trellis/spec/` 下的开发指南

### 清理

- [ ] 删除 `server/internal/pkg/sso/` 整个包
- [ ] 删除 `server/internal/pkg/jwt/validator.go`
- [ ] **保留并简化** `server/internal/pkg/token/blacklist.go`（仅用于紧急吊销）
- [ ] **保留并简化** `server/internal/modules/auth/user_sync.go`（shadow user 同步）
- [ ] 删除 L1+L2 用户缓存（`pkg/sso/cache.go`）
- [ ] **保留 `users` 表**（shadow user，业务外键锚点），删除 RBAC 6 张表
- [ ] Bearer token 走 Zitadel introspection，Cookie 走短 TTL + refresh
- [ ] 更新 OpenAPI spec

## Acceptance Criteria

- [ ] 用户可通过 Zitadel 登录（邮箱+密码、手机号+验证码）
- [ ] Token claim 携带角色和组织（不携带认证状态元数据）
- [ ] 管理后台根据 Token 角色展开的能力控制页面/按钮可见性
- [ ] 志愿者可删帖但不能看发帖人实名信息（OpenFGA 区分）
- [ ] 学校管理员可管理本校内容（OpenFGA 关系链验证）
- [ ] 超级管理员可管理全生态（OpenFGA 继承）
- [ ] 实名认证和学生认证状态真相源在应用 DB，Zitadel 只存粗角色
- [ ] 已认证学生的 verified_student 角色在 Zitadel 中分配，Token 自动携带
- [ ] shadow user 表保留，业务外键链完整
- [ ] Bearer token 走 Zitadel introspection 实现即时吊销
- [ ] 所有旧 Casdoor 代码和 RBAC 6 张表已删除
- [ ] `go build` / `go test` / 前端 lint 全部通过
- [ ] 架构文档已更新
...
- 学生认证：结果存应用 DB（真相源），异步同步 Zitadel 粗角色（verified_student）
- 认证撤销：应用 DB 立即更新 + 异步移除 Zitadel 角色
