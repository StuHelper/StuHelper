...
- [ ] 为每个 admin 路由定义所需的权限名称
- [ ] 在路由注册时应用 `RequirePermission(...)` 中间件
- [ ] 确保权限检查逻辑真正生效
...
- [ ] 确认数据库 schema 中 `verification_status` 的定义
- [ ] 如果已经是正确的 enum，确保查询逻辑正确
- [ ] 如果不是，需要添加 migration 或调整查询逻辑

### R3: 修复前端文案判断逻辑

- [ ] 确认后端返回的所有可能的 `verifyMethod` 值
- [ ] 修正前端判断逻辑
- [ ] 确保所有情况都有正确的文案

### R4: 修复 admin logs 路由引用

- [ ] 确认 `admin-logs` 路由是否存在
- [ ] 如果不存在，移除引用或创建占位页面

## Acceptance Criteria

- [ ] 所有 admin 路由都应用了细粒度权限检查
- [ ] 权限检查逻辑在运行时真正生效
- [ ] 实名认证状态查询返回正确的结果
- [ ] 前端文案判断逻辑正确
- [ ] Admin logs 路由引用问题已解决
- [ ] 所有修改通过 lint 和 typecheck
- [ ] 手动测试验证功能正常
