...
本文档定义了 StuHelper 项目的开发规范，包括代码风格、Git 工作流、API 设计等方面的约定。

**项目仓库**: https://gitea.stuhelper.com/StuHelper/StuHelper

## 目录

- [StuHelper 开发规范](#stuhelper-开发规范)
  - [一、Git 管理规范](#一git-管理规范)
  - [二、Commit Message 规范](#二commit-message-规范)
  ...
  - [八、日志规范](#八日志规范)
  - [九、工具链强制约束](#九工具链强制约束)
  - [附录：Uber Go 语言编码规范](#附录uber-go-语言编码规范)
...
| `develop`   | 开发主分支。包含最新特性，**受保护**。               | -                       |
| `feature/*` | 功能分支。从 develop 切出，合并回 develop。          | `feature/模块-功能描述` |
| `fix/*`     | 修复分支。从 develop 切出，合并回 develop。          | `fix/模块-问题描述`     |
| `hotfix/*`  | 紧急修复。从 main 切出，同时合并回 main 和 develop。 | `hotfix/问题描述`       |
| `release/*` | 发布准备                                             | `release/v1.0.0`        |

**分支命名示例**：

```
feature/course-review-vote
...
- `main` 和 `develop` 分支禁止直接推送
- 所有变更必须通过 Pull Request 合并
- PR 合并前必须通过 CI 检查和 Code Review
...
按模块划分：`user`, `identity`, `course-review`, `permission`, `audit`, `api`, `web`, `docs`

将 scope-enum 设置为宽松模式，或者在 CI 中配置脚本自动读取 internal/modules 下的文件夹名作为合法的 scope。
...
```
feat(course-review): add vote functionality for reviews
...
feat(agent): support websocket heartbeat
```
...
```
update code                    # 无意义描述
bug fix                        # 缺少 scope 和具体描述
修复了一些问题                   # 使用中文，描述模糊
fix: Fix bug                   # 重复，描述不具体
WIP                            # 不完整的提交
```

### 2.5 破坏性变更

在 footer 中添加 `BREAKING CHANGE:` 说明：

```
feat(api): change response format for course list
...
- **单一职责**：一个 PR 只做一件事，不要把"修复 Bug A"和"开发功能 B"混在一个 PR 里
- **代码行数**：PR 代码变更最好控制在 **400 行以内**，太多了 Reviewer 看不过来
- **自我检查**：提交 PR 前，作者必须自己过一遍代码，删掉 `console.log` 和注释掉的代码
- **CI 通过**：自动化测试（Lint, Build, Test）必须全绿才能请求 Review

### 3.4 审批规则

- 至少需要 **1 位核心开发者** Approve 才能合并
- **禁止 Self-Merge**（自己合并自己的 PR），除非是紧急 hotfix 且无人可 Review
- 合并方式：优先使用 **Squash Merge** 保持提交历史整洁

### 3.5 Review 响应时间

- 普通 PR：24 小时内完成首次 Review
- 紧急 PR：4 小时内完成 Review
- 作者应在 24 小时内响应 Review 意见
...
由于 Go、Vue 和数据库有不同的命名惯例，需要在各层之间进行转换：

| 层级                | 命名风格   | 示例                                  |
| ------------------- | ---------- | ------------------------------------- |
| 数据库 (PostgreSQL) | snake_case | `user_id`, `created_at`, `is_deleted` |
| Go 结构体           | PascalCase | `UserID`, `CreatedAt`, `IsDeleted`    |
| Go JSON Tag         | camelCase  | `userId`, `createdAt`, `isDeleted`    |
| Vue/TypeScript      | camelCase  | `userId`, `createdAt`, `isDeleted`    |
| API 请求/响应       | camelCase  | `userId`, `createdAt`, `isDeleted`    |
...
| -------- | ------------ | ----------- | ----------- |
| 主键     | `id`         | `ID`        | `id`        |
| 用户 ID  | `user_id`    | `UserID`    | `userId`    |
| 创建时间 | `created_at` | `CreatedAt` | `createdAt` |
| 更新时间 | `updated_at` | `UpdatedAt` | `updatedAt` |
| 删除时间 | `deleted_at` | `DeletedAt` | `deletedAt` |
| 是否删除 | `is_deleted` | `IsDeleted` | `isDeleted` |
| 排序号   | `sort_order` | `SortOrder` | `sortOrder` |
...
- 全小写，下划线分隔：`user_service.go`、`review_handler.go`
- 测试文件：`user_service_test.go`

**结构体/接口命名**：

- 结构体：PascalCase（大驼峰），如 `User`、`UserService`
- 接口：以 `er` 结尾，如 `Reader`、`Writer`、`UserRepository`

**变量命名**：

- 局部变量越短越好：`i`、`v`、`req`、`resp`
- 全局变量或参数需描述清楚：`userID`、`dbConn`、`httpClient`
...
// ✅ 使用 fmt.Errorf 包装错误，携带上下文 (Go 1.13+)
if err != nil {
...
**文件命名**：PascalCase，如 `UserProfile.vue`、`CourseList.vue`

**组件名称必须由多个单词组成**，防止与 HTML 标签冲突：

- ✅ `TodoItem.vue`、`AppButton.vue`、`UserCard.vue`
- ❌ `Item.vue`、`Button.vue`、`Card.vue`

**Props/Emit 命名**：

- Props 定义使用 camelCase（驼峰）
- Template 中使用 kebab-case（短横线）

```vue
<!-- 父组件 -->
<user-card :user-id="123" @update-info="handleUpdate" />
```
...
// 4. 计算属性
const displayName = computed(() => props.userId);
...
```typescript
// api/course-review.ts
import request from "@/utils/request";
import type { Course, Review } from "@/types";

export const getCourseList = (params: { page: number; size: number }) => {
	return request.get<Course[]>("/api/v1/course-review/courses", { params });
};

export const postReview = (data: Partial<Review>) => {
	return request.post<Review>("/api/v1/course-review/reviews", data);
};
```
...
**强制使用 TypeScript**：全员强制使用 `<script setup lang="ts">`，禁止使用 Options API。
...
**Ref vs Reactive 使用建议**：

- 基础类型（string, number, boolean）用 `ref`
- 对象/数组建议统一用 `ref`，保持一致性
...
**Scoped 样式**：所有组件样式必须加 `<style scoped>`，防止样式污染。

**类名命名**：使用 BEM 命名法或 kebab-case：
...
**Tailwind CSS**：如果使用 Tailwind，尽量使用 `@apply` 抽取复杂样式，避免 HTML 中 class 堆积过长。
...
使用 Husky 管理 Git Hooks，lint-staged 只检查暂存的文件：
...
### 9.4 Commitlint (提交信息检查)

使用 Commitlint 强制检查 commit message 格式：
...
> 如果 commit message 不符合 `feat: xxx` 等格式，提交将被拒绝。
...
请看指南 [对于未导出的顶层常量和变量，使用\_作为前缀](#对于未导出的顶层常量和变量使用_作为前缀)。
