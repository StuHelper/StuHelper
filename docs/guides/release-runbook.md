...
- StuHelper Admin break-glass 提权：`built-in/admin` 只用于 Casdoor 系统级维护，不能登录 `stuhelper` 组织的 StuHelper 业务后台。若生产没有任何可用 `super_admin`，在 Casdoor/PostgreSQL 节点执行 `STUHELPER_INITIAL_SUPER_ADMINS=<casdoor-username> ./infra/ops/casdoor-grant-super-admin.sh`，脚本会幂等把 `stuhelper` 组织内已有用户加入 `super_admin` role 的 `users` 列表。Casdoor `user.is_admin=true` 不等于 StuHelper 后台权限；后台只信任 OIDC `roles` claim 中的 `super_admin` / `school_admin` 等 flat role。授权后受影响用户必须退出 StuHelper 和 SSO 并重新登录，拿到包含新 role 的 ID token。该操作属于 break-glass，应在发布记录中写明操作者、用户名、原因和验证结果。
