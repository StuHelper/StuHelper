...
- [ ] 公网 SSO 和入群验证入口门禁已核对：默认 `PUBLIC_INGRESS_CONFIG_PREFLIGHT_ENABLED=true`，远端 preflight / prod deploy 会先审计本机宝塔 Nginx 主站和 join 配置；默认 `PUBLIC_INGRESS_PREFLIGHT_ENABLED=true`，随后验证 `stuhelper.com`、`join.stuhelper.com`、`sso.stuhelper.com` 公共 DNS-over-HTTPS 有公网 A/AAAA 且 TLS 可达。DoH 检查按 `PUBLIC_INGRESS_PUBLIC_DNS_RESOLVERS` 的 HTTPS 端点列表做有界故障转移：只在传输失败时尝试下一端点，任一可达解析器返回 NXDOMAIN、无地址、私网地址或畸形 JSON 都立即失败关闭，不能用后备解析器掩盖真实 DNS 漂移。`sso.stuhelper.com` discovery/JWKS/authorize 路由必须反代到 Casdoor。旧 identity smoke 已删除；默认 `ADMISSION_PUBLIC_SMOKE_ENABLED=true`，远端 preflight / prod deploy 都会运行 `admission-public-smoke.sh` 并写入 `infra/generated/admission-public-smoke-evidence.json`，要求 `join.stuhelper.com/verify/<probe>` 和 `/student-verification/manual-camera/<probe>` 由 Web SPA 承载，前者禁用 camera、后者允许 camera；`join.stuhelper.com/api/v1/metrics/vitals` 和 `/api/v1/metrics/frontend-errors` 接受同源 beacon 并返回 204；旧 `/admission/freshman/camera/*` 与 `/api/v1/admission/freshman/camera-handoffs/*` 返回 404；`join.stuhelper.com/`、`join.stuhelper.com/developers/apps`、`join.stuhelper.com/verify`、主站 `/verify`、主站 `/verify/*` 全部返回 404。生产仍要求 `ADMISSION_PUBLIC_SMOKE_ALLOW_LOCAL_TARGETS=false`、`ADMISSION_PUBLIC_SMOKE_CURL_INSECURE=false`。默认 `PUBLIC_WEB_AUTH_BROWSER_SMOKE_ENABLED=true`，prod deploy 会运行 `public-web-auth-browser-smoke.mjs`，用 Playwright 真浏览器验证主站登录页非空、开发者入口未登录跳回 `/login?redirect=/developers/apps`、点击登录进入 `sso.stuhelper.com/login/oauth/authorize` 并看到账号密码登录和 `/signup/oauth/authorize` 注册入口、点击注册进入 `sso.stuhelper.com/signup/oauth/authorize` 并看到账号密码注册表单、join 根路径/主站业务路径不渲染主站内容、join verify SPA 可加载，并验证 student-verification 手动拍照页允许 camera。
- [ ] 启用 Campus Connector Gateway 时，`connector.stuhelper.com` 已解析到主站边缘，`CAMPUS_CONNECTOR_GATEWAY_PUBLIC_PORT=9444`、`CAMPUS_CONNECTOR_GATEWAY_EXTERNAL_PORT=19444`、`CAMPUS_CONNECTOR_ALLOWED_SOURCE_CIDRS` 为批准的稳定校园节点 IPv4 CIDR，且 `NGINX_PUBLIC_INGRESS_PROFILE=app-all`。`/www/server/panel/vhost/nginx/tcp/connector.stuhelper.com.conf` 必须被实际 `nginx -T` 加载，只做原始 stream TCP 透传到 `127.0.0.1:19444`，没有任何 Nginx TLS 终止；Nginx `allow`/`deny all` 与 UFW 规则均不得开放 `0.0.0.0/0`。DNS 解析成功不能替代 listener、防火墙、mTLS heartbeat 或真实 Connector E2E。
...
- Campus Connector：`NGINX_PUBLIC_INGRESS_PROFILE=app-all ./infra/ops/nginx-public-ingress-preflight.sh` 通过；`connector.stuhelper.com:9444` 只从批准 CIDR 可达并把 TLS 原样送到 Gateway；随后必须出现真实节点 mTLS heartbeat。端口可达或 TLS 握手被拒绝都不能单独证明 LDAP/Oracle operation 已健康。
...
- 公网入群验证入口：`./infra/ops/admission-public-smoke.sh`，留档 `infra/generated/admission-public-smoke-evidence.json`；同时验证 join 域根路径和主站业务路径返回 404、`/api/v1/metrics/vitals` 与 `/api/v1/metrics/frontend-errors` 同源 beacon 返回 204、专用 student-verification 手动拍照页允许 camera、普通 verify 页面禁用 camera，以及旧 freshman camera 页面和 SSE 端点均返回 404。
...
- 北航学籍数据准入：学校目录表不是认证白名单；只有 `school_verification_profiles.enabled=true` 且
  ...
  只读取证：后者只允许使用用户明确指定的既有账号执行仓库固定的 Oracle `SELECT`，不得接受任意 SQL，
  也不得输出账号、DSN 或学生原始行；二者都不能作为当前 Campus Connector 或新学生认证域的上线证据，
  更不能作为连接器不可用时的 fallback。`import-buaa-academic-students.sh` 会写入旧本地表，只保留历史版本
  排障能力，不得用于新认证域或当前生产切换。只有
  `provision-external-student-source-oracle-readonly.sh` 是明确拒绝执行的兼容占位：它不会连接 Oracle，
  也不会创建、授权、轮换或修改账号。当前生产只能把用户明确指定的既有 Oracle 账号配置在校园连接器
  secret store。
...
- admission 聚合生产证据：`./infra/ops/admission-mvp-production-evidence.sh` 继续留档 SSO、公网入口、Web
  ...
  发布证据还必须包含有效 Campus Connector 节点/operation、未过硬失效阈值的 active roster snapshot、
  学生认证凭据派生资格，以及凭据撤销后 admission 回退。普通聚合证据允许 skipped，只能作为 smoke；
  最终上线仍必须在主站使用 `make prod-admission-mvp-final-evidence`，并在 Koishi 节点使用
  `make prod-admission-mvp-final-koishi-evidence`，两者均不得含 failed/skipped。
