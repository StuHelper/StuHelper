...
- 北航学籍数据准入：生产推荐启用外部只读 Oracle 学籍源，secret env 配置 `EXTERNAL_STUDENT_SOURCE_ENABLED=true`、`EXTERNAL_STUDENT_SOURCE_PROVIDER=oracle`、`EXTERNAL_STUDENT_SOURCE_SCHOOL_CODE=4111010006` 和 Oracle 连接/表/列变量。该源只读取 `USR_JWBIZ.T_XS_JBXX` 的 `XH` 与 `XM`，用于北航“学号 + 姓名”即时匹配；真实密码不得写入仓库或 runbook。学校目录导入只写 `schools`，不是白名单；管理后台学校配置页应展示所有已录入学校，缺少 `school_configs` 的目录学校按停用展示，只有 `school_configs.enabled=true` 才进入学生认证和 admission 白名单，当前只应启用北航。`./infra/ops/admission-production-readiness.sh` 通过后应在 summary 中显示 `buaa_student_source=external_oracle`，随后运行 `./infra/ops/external-student-source-smoke.sh` 并留档 `infra/generated/external-student-source-smoke.json`，证明 Oracle 可连接、配置表可读且至少存在一条 `XH/XM` 非空记录。需要抽样校验时，只在 secret env 中设置 `EXTERNAL_STUDENT_SOURCE_SMOKE_REQUIRE_SAMPLE=true`、`EXTERNAL_STUDENT_SOURCE_SMOKE_STUDENT_ID` 和可选 `EXTERNAL_STUDENT_SOURCE_SMOKE_EXPECTED_NAME`；evidence 只记录学号哈希前缀和匹配布尔值，不记录原始学号、姓名或密码。如果暂时没有可用 Oracle 外部源，才使用本地 fallback 表 `academic.buaa_students`；拿到真实 TSV 后先运行 `BUAA_ACADEMIC_VALIDATE_ONLY=true BUAA_ACADEMIC_STUDENTS_TSV=/path/to/buaa-students.tsv ./infra/ops/import-buaa-academic-students.sh` 做离线校验，再运行 `BUAA_ACADEMIC_STUDENTS_TSV=/path/to/buaa-students.tsv ./infra/ops/import-buaa-academic-students.sh` 导入。TSV 至少包含 `xh` 和 `xm` 列，也接受 `学号` 和 `姓名` 列名；脚本幂等 upsert，不清空旧数据，不打印学生明细。导入后重新运行 readiness 和 external source smoke。
