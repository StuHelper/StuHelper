...
- StuHelper Admin break-glass 提权：`built-in/admin` 只用于 Casdoor 系统级身份维护，不能直接获得 StuHelper 业务后台权限。首次切换或授权账本灾难恢复时，在迁移完成后执行 `STUHELPER_INITIAL_SUPER_ADMINS=<casdoor-username-1,casdoor-username-2> ./infra/ops/authorization-bootstrap-super-admin.sh`。目标用户必须已登录过一次并存在于 StuHelper `users` 表；生产少于两个去重后的目标会直接失败。脚本以 `authorization-bootstrap` system actor 在一个 PostgreSQL 事务内写全部 `authorization_grants`、安全审计和 OpenFGA outbox，任何目标/审计/outbox 失败都整体回滚；它不修改 Casdoor role membership，也不直接写 OpenFGA tuple。只要账本中已有任一 desired `super_admin`，脚本就整体跳过，避免日常部署复活已撤销主体。执行后在 `/authorization/grants` 确认两条 grant 均有 `activatedAt` 且 projection 为 `applied`，再分别用两个独立控制的账号完成登录、后台入口和 step-up mutation 验收。该操作属于 break-glass，应在发布记录中写明宿主机操作者、目标用户名、原因、grant ID 和投影验证结果；账本内审计不会把任一目标用户伪记为执行该 shell 的操作者。
- StuHelper Admin 初始 MFA bootstrap：生产后台对 `super_admin` / `school_admin` 还要求 StuHelper 本地 `user_mfa_enrollment.active=true` 和当前登录 token 的 MFA proof。若登录后台出现 `A0010204`，说明 PostgreSQL 授权 grant 已通过但本地 MFA enrollment 缺失。先确认用户已在 `sso.stuhelper.com/account` 绑定 SMS/App/WebAuthn/TOTP 类 MFA，然后在主站生产节点执行 `STUHELPER_ADMIN_MFA_BOOTSTRAP_USERS=<casdoor-username> ./infra/ops/bootstrap-admin-mfa-enrollment.sh`。脚本会确认 StuHelper `authorization_grants` 中存在 desired `super_admin` 或 `school_admin`、Casdoor 身份仍有效且已有 SMS/App/WebAuthn/TOTP MFA 证据，再幂等 upsert `user_mfa_enrollment` 并写入 `iam.mfa.bootstrap` 审计事件；Casdoor 角色在整个流程中不参与授权。执行后受影响用户必须退出 StuHelper 和 SSO 并重新登录，让新 token 带上 MFA proof；如果随后返回 `A0010205`，按前端 step-up 跳转重新完成一次 SSO MFA。
