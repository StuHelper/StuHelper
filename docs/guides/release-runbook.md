...
- 北航学籍数据准入：`academic.buaa_students` 不能是空表，否则北航学号姓名校验和 admission 老生邮箱 OTP 必然失败。拿到真实 TSV 后运行 `BUAA_ACADEMIC_STUDENTS_TSV=/path/to/buaa-students.tsv ./infra/ops/import-buaa-academic-students.sh`。TSV 至少包含 `xh` 和 `xm` 列，也接受 `学号` 和 `姓名` 列名；脚本幂等 upsert，不清空旧数据，不打印学生明细。导入后重新运行 `./infra/ops/admission-production-readiness.sh`。
