...
- StuHelper Admin break-glass 提权：`built-in/admin` 只用于 Casdoor 系统级维护，不能登录 `stuhelper` 组织的 StuHelper 业务后台。若生产没有任何可用 `super_admin`，在 Casdoor/PostgreSQL 节点执行 `STUHELPER_INITIAL_SUPER_ADMINS=<casdoor-username> ./infra/ops/casdoor-grant-super-admin.sh`，脚本会幂等把 `stuhelper` 组织内已有用户加入 `super_admin` role 的 `users` 列表；Casdoor role membership 的实际格式必须是 `organization/username`，例如 `stuhelper/Xauryan`，脚本会清理同一用户的旧裸用户名成员。Casdoor `user.is_admin=true` 不等于 StuHelper 后台权限；后台只信任 OIDC `roles` claim 中的 `super_admin` / `school_admin` 等 flat role。授权后受影响用户必须退出 StuHelper 和 SSO 并重新登录，拿到包含新 role 的 ID token。该操作属于 break-glass，应在发布记录中写明操作者、用户名、原因和验证结果。
- StuHelper Admin 初始 MFA bootstrap：生产后台对 `super_admin` / `school_admin` 还要求 StuHelper 本地 `user_mfa_enrollment.active=true` 和当前登录 token 的 MFA proof。若登录后台出现 `A0010204`，说明角色已通过但本地 MFA enrollment 缺失。先确认用户已在 `sso.stuhelper.com/account` 绑定 SMS/App/WebAuthn/TOTP 类 MFA，然后在主站生产节点执行 `STUHELPER_ADMIN_MFA_BOOTSTRAP_USERS=<casdoor-username> ./infra/ops/bootstrap-admin-mfa-enrollment.sh`。脚本会确认 Casdoor 用户属于 `stuhelper` 组织、已有 `super_admin` 或 `school_admin` role membership、已有 SMS/App/WebAuthn/TOTP MFA 证据，再幂等 upsert `user_mfa_enrollment` 并写入 `iam.mfa.bootstrap` 审计事件。执行后受影响用户必须退出 StuHelper 和 SSO 并重新登录，让新 token 带上 MFA proof；如果随后返回 `A0010205`，按前端 step-up 跳转重新完成一次 SSO MFA。
...
- 北航学籍数据准入：生产推荐启用外部只读 Oracle 学籍源，secret env 配置 `EXTERNAL_STUDENT_SOURCE_ENABLED=true`、`EXTERNAL_STUDENT_SOURCE_PROVIDER=oracle`、`EXTERNAL_STUDENT_SOURCE_SCHOOL_CODE=4111010006` 和 Oracle 连接/表/列变量。该源只读取 `USR_JWBIZ.T_XS_JBXX` 的 `XH` 与 `XM`，用于北航“学号 + 姓名”即时匹配；真实密码不得写入仓库或 runbook。学校目录导入只写 `schools`，不是白名单；管理后台学校配置页应展示所有已录入学校，缺少 `school_configs` 的目录学校按停用展示，只有 `school_configs.enabled=true` 才进入学生认证和 admission 白名单，当前只应启用北航。主站不得使用 SYSTEM/SYS 连接学籍源；在 Oracle 源端用 `EXTERNAL_STUDENT_SOURCE_ORACLE_READONLY_PASSWORD=<secret> ./infra/ops/provision-external-student-source-oracle-readonly.sh` 创建或轮换只读用户，只授予 `CREATE SESSION` 和目标表 `SELECT`。首选执行入口是 `./infra/ops/admission-student-source-go-live.sh`：外部源模式会先运行 `external-student-source-smoke.sh`，本地 TSV 模式会先校验再导入 `BUAA_ACADEMIC_STUDENTS_TSV`，最后统一运行 `admission-production-readiness.sh`。底层导入脚本仍是 `./infra/ops/import-buaa-academic-students.sh`，用于单独离线校验或排查 TSV 格式问题。`./infra/ops/admission-production-readiness.sh` 通过后应在 summary 中显示 `buaa_student_source=external_oracle`，随后运行 `./infra/ops/external-student-source-smoke.sh` 并留档 `infra/generated/external-student-source-smoke.json`，证明 Oracle 可连接、配置表可读且至少存在一条 `XH/XM` 非空记录。需要抽样校验时，只在 secret env 中设置 `EXTERNAL_STUDENT_SOURCE_SMOKE_REQUIRE_SAMPLE=true`、`EXTERNAL_STUDENT_SOURCE_SMOKE_STUDENT_ID` 和可选 `EXTERNAL_STUDENT_SOURCE_SMOKE_EXPECTED_NAME`；evidence 只记录学号哈希前缀和匹配布尔值，不记录原始学号、姓名或密码。如果暂时没有可用 Oracle 外部源，才使用本地 fallback 表 `academic.buaa_students`；拿到真实 TSV 后运行 `BUAA_ACADEMIC_STUDENTS_TSV=/path/to/buaa-students.tsv ./infra/ops/admission-student-source-go-live.sh`，脚本会先离线校验 TSV 再幂等导入并复跑 readiness。TSV 至少包含 `xh` 和 `xm` 列，也接受 `学号` 和 `姓名` 列名；脚本幂等 upsert，不清空旧数据，不打印学生明细。
