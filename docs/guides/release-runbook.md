...
- [ ] 关键变量已核对：`POSTGRES_PASSWORD`、`REDIS_PASSWORD`、`TAG`、`OBJECT_STORAGE_*`、`WEB_VITE_SSO_URL=https://id.stuhelper.com`、`WEB_VITE_IDENTITY_URL=https://id.stuhelper.com`、`WEB_VITE_WEB_URL=https://stuhelper.com`
...
`baota-stuhelper.conf` 用于主站机器的 `stuhelper.com` / `id.stuhelper.com`。Casdoor upstream 通过 `id.stuhelper.com` 的 `/login/oauth/*`、`/signup/oauth/*`、`/api/*` 和静态资源路径反代到本机 `127.0.0.1:8087`；不要把 `sso.stuhelper.com` 当作默认公网入口。
...
远端发布主机默认使用 `NGINX_PUBLIC_INGRESS_PROFILE=stuhelper`，只审计本机拥有的 `stuhelper.com` / `id.stuhelper.com` server block。历史兼容的 `NGINX_PUBLIC_INGRESS_PROFILE=sso` 只用于显式保留独立 `sso.stuhelper.com` 公网入口的环境。
