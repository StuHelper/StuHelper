...
- [ ] 生产机上的逻辑备份 / base backup / backup sync timer 已启用；升级部署 bundle 后已由 root 重新运行 `./infra/ops/install-backup-timers.sh`，三个 service 的完整有效环境与安装器定义精确一致（包括 `BACKUP_OBJECT_STORAGE_OFF_HOST_REQUIRED=true`，且没有额外进程控制变量）。
