...
备份目标还必须与生产主机处于独立故障域。完成异机/云对象存储配置并验证生产主机完全丢失后仍可取回工件，才可把 `BACKUP_OBJECT_STORAGE_OFF_HOST_CONFIRMED` 设为 `true`。同时必须用 `BACKUP_OBJECT_STORAGE_LOCAL_IDENTITY_CIDRS` 列出所有可路由回本机的公网、1:1 NAT 或负载均衡 IP/CIDR；确认不存在额外身份时也要显式填写 `none`。生产预检会拒绝缺少这份清单、只有逗号/空白等分隔符的空清单或命中清单的端点，也会拒绝 `minio`、`object-storage` 等单标签 Compose 主机名、loopback、link-local、旧式缩写数字 IPv4、带 zone identifier 的 IPv6、尾随点 FQDN 和 `.local` 端点。唯一的 link-local 例外是显式设置 `BACKUP_OBJECT_STORAGE_PROVIDER_PRIVATE_ENDPOINT=tencent-cos`：它只接受 `TencentCOS` provider、官方 regional COS service hostname、匹配的 region、完整 `BucketName-APPID`、virtual-hosted addressing、默认 HTTPS 端口及公开 CA 证书校验。这个模式只表示同地域 CVM 正在使用 COS provider-private service VIP，不能替代异机取回证据，也不会自动设置 `BACKUP_OBJECT_STORAGE_OFF_HOST_CONFIRMED=true`。
...
宝塔 Nginx 的有效配置可能引用只有 root 才能打开的日志、证书、PID 或 vhost 目录，因此非 root 部署账号直接执行 `nginx -T`、或再次打开 Connector TCP vhost，都会在解析阶段失败。此时必须由 root 在预检前即时生成一份只读的完整 `nginx -T` 快照，设为 `root:<部署组>`、模式 `0640`，再以 `NGINX_PUBLIC_INGRESS_CONFIG_FILE=/绝对路径/快照` 和 `BACKUP_TIMERS_ACTIVATE=true` 调用备份 timer 安装器。Connector 审计从快照内精确匹配的 `# configuration file <path>:` 分段读取原始 stream 配置，并拒绝缺失或重复分段；不要求部署账号遍历宝塔的 root-only vhost 目录。安装器只把这个可选路径加入隔离的 activation-preflight 环境；远端预检在 Vault 配置投影期间保留调用方提供的路径，并继续对快照执行完整 ingress 审计，不得改用 `PUBLIC_INGRESS_CONFIG_PREFLIGHT_ENABLED=false` 绕过门禁。激活预检不再用 `runuser -G` 手工重建 NSS 组列表，而是由 `systemd-run` 创建 `Type=exec`、`--wait`、`--pipe`、`--collect` 的瞬时 service，并与正式备份 service 使用同一组 `User`、`Group`、`WorkingDirectory` 和动态加载器环境清理规则。因此辅助组完全由同一 systemd manager 按正式 unit 语义初始化：保留 service 实际具备的 `docker` 组，不会误带入被 `DEPLOY_GROUP` 覆盖的账号原始主组，也没有 GID→组名→GID 的二次 NSS 解析歧义。瞬时 unit 失败必须原样返回非零状态，且只有预检成功后才能启用 timer。激活完成后应删除临时快照；后续预检必须重新生成，不能复用可能陈旧的配置。
...
首次 bootstrap 即使尚未上传 deploy bundle，也会预先创建由部署用户拥有的 `/var/lib/stuhelper/postgres/wal-restore`。因此后备 systemd 单元与后续部署用户执行的远端预检使用同一 local-state 布局。若 bundle 尚不存在，bootstrap 只写入后备 unit，不启用或启动 timer，避免 15 分钟同步任务在脚本与生产配置缺失时留下失败 `Result`。正式安装器默认也只写 unit、保持现有激活状态不变；首个 bundle、生产配置和 Vault 就绪后，root 必须显式执行 `BACKUP_TIMERS_ACTIVATE=true ./infra/ops/install-backup-timers.sh`。安装器先用隔离环境切换为非 root 部署用户执行 `remote-preflight.sh --timer-activation`，并把写入 unit 的同一个 `BACKUP_STAGING_DIR` 显式传入该隔离进程；默认目录或受支持的独立备份磁盘都必须按实际有效路径做精确环境比对。该阶段验证完整静态配置、Vault、异机对象存储、备份工具和 unit 契约，只允许 timer 暂未 enabled/active，并对尚未启动的本机 PostgreSQL 延后连通性检查；全部通过后才 `enable --now`。预检还枚举所有 `*-postgres-{dump-backup,basebackup,backup-sync}.timer`，只允许规范的三条 `stuhelper-*` unit；历史 `SYSTEMD_PREFIX=custom` 等遗留 unit 即使 disabled 也必须由 root 在确认名称后显式 `disable --now` 并移除 unit 文件、`daemon-reload`，不能让两套日历并存。安装器不会为了重装先 `disable --now` 或清除已有 service 失败结果，因此 Vault 封印、对象存储暂不可用或真实备份失败会让安装器失败关闭，同时保留原有 timer 的 enabled/active 状态和诊断证据。三个规范 timer 都通过统一的受保护调度包装器运行：首个 `current-release.env` 尚未写入时，只有同时不存在 `releases.log`、per-tag 记录、外部 PostgreSQL 选择、本机 PostgreSQL 容器和 data volume，且 Docker daemon 可读地证明这些条件，任务才以“尚无已提交发布或 datastore 证据”成功延期。已有 datastore 仍会失败关闭；唯一受支持的恢复例外是部署用户在隔离 PITR 演练后显式运行 `activate-existing-postgres-backups.sh`。该控制器与生产部署共用同一主机锁，在任何持久写入前核对固定 PostgreSQL digest、Compose project/service、data/WAL volume、实时 `system_identifier`、WAL 归档和异机目标；随后强制生成全新的 logical/base 工件，完成 logical/base/WAL 同步，再从异机目标取回这两份新工件并核对文件名和 SHA256。证据中的每份工件必须不早于本次激活尝试；额外的 72 小时年龄上限覆盖受保护备份与传输单元的容量窗口，而不是把多阶段流程错误压缩到一小时。全部通过后才原子发布 `postgres-backup-activation.json` 及 `postgres-backup-activations/<activation-id>.json`，记录同时绑定两份恢复工件名称与摘要。当前指针必须字节匹配最新不可变记录；所有记录都由部署用户持有且为 `0600`，并通过 predecessor ID 与前任文件 SHA256 形成从当前回溯到首条记录的完整无环链。调度器每轮重新绑定全部实时身份；身份漂移先失败关闭，完成新的隔离恢复审计后可显式重跑控制器，它必须先验证旧链、再生成和取回全新恢复工件，最后追加 superseding 记录并原子切换当前指针。孤儿记录、缺失前任、摘要不符、循环、时间倒序或直接改写历史均拒绝恢复。任何残存 release log/per-tag 记录、外部 PostgreSQL 选择、symlink、权限或身份漂移都拒绝静默激活，不能用该记录伪造应用发布。正常 `current-release.env` 一旦存在仍优先使用并要求匹配不可变 per-tag 记录；随后数据库、对象存储或同步失败继续失败关闭。常规部署前/后预检仍要求三个 timer 已启用且 active，并要求当前 service `Result=success`。
