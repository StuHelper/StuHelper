...
宝塔 Nginx 的有效配置可能引用只有 root 才能打开的日志、证书或 PID 路径，因此非 root 部署账号直接执行 `nginx -T` 会在解析阶段失败。此时必须由 root 在预检前即时生成一份只读的完整 `nginx -T` 快照，设为 `root:<部署组>`、模式 `0640`，再以 `NGINX_PUBLIC_INGRESS_CONFIG_FILE=/绝对路径/快照` 和 `BACKUP_TIMERS_ACTIVATE=true` 调用备份 timer 安装器。安装器只把这个可选路径加入隔离的 activation-preflight 环境；远端预检在 Vault 配置投影期间保留调用方提供的路径，并继续对快照执行完整 ingress 审计，不得改用 `PUBLIC_INGRESS_CONFIG_PREFLIGHT_ENABLED=false` 绕过门禁。激活预检会把 `DEPLOY_GROUP` 设为进程主组，同时只从部署用户的 NSS 账号组列表重建辅助组；这使快照权限与 systemd service 一致，又保留访问 root-owned Docker socket 所需的 `docker` 组，不会继承调用方或 root 的组。NSS 列表包含 GID 0、非数字 GID，或 `getent` 返回的组记录与该 GID 不一致时都会失败关闭，避免预检获得正式 systemd service 不具备的 root 组权限。激活完成后应删除临时快照；后续预检必须重新生成，不能复用可能陈旧的配置。
