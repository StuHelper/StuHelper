...
- `yarn test:ui` 会先构建工作区，再用临时 SQLite 和临时群管中心 JSON 数据目录拉起 Koishi Console，通过 Playwright 覆盖群管中心 NavRail、11 个业务视图、ChatDock、全局搜索、配置治理、订阅、黑名单、警告、日志检索、全局设置和系统缓存真实交互，并检查 `pageerror`、未放行的 console error/warning、关键资源加载失败和关键资源 HTTP 4xx/5xx。
...
- 当前自动化已覆盖：
  ...
  - 工作区运行时入口与启动烟雾验证
  - 群管中心 WebUI 的 NavRail、11 个业务视图、ChatDock、全局搜索、配置治理、订阅、黑名单、警告、日志检索、全局设置和系统缓存 Playwright UI smoke，并把 `document`、`script`、`stylesheet`、`font`、`image` 关键资源加载失败和 HTTP 4xx/5xx 视为失败
  - 管理员命令与命令权限表初始化
  - 群模板、群绑定和数据库优先级策略解析
