...
| 事实               | 数据来源                                                                            | 用途                     |
| ------------------ | ----------------------------------------------------------------------------------- | ------------------------ |
| `studentVerified`  | `user_profiles.verification_status == 'verified'` 且 `school_id` 命中评课访问白名单 | 评课完整内容可见性       |
...
| 已认证但未通过学生认证 | 评论标题和内容按后台策略截断预览                     |
| 已通过学生认证         | 显示评论完整内容                                     |
| 发布评课               | 需要学生认证通过且实名认证通过                       |
| 管理操作               | 由能力字符串控制入口和可用操作；管理员可查看隐藏内容 |

评课访问白名单和预览裁剪参数来自 `system_configs`：

- `review_access_school_ids`
- `review_preview_title_chars`
- `review_preview_content_chars`
- `review_preview_content_percent`
