    ...
    <div>
        ...
        <el-table v-loading="loading" :data="list" border stripe>
            ...
            <el-table-column label="认证状态" width="110">
                <template #default="{ row }">
                    <el-tag
                        :type="statusTagType(row.verificationStatus)"
                        size="small"
                    >
            ...
            <el-table-column label="补充信息" min-width="220">
                <template #default="{ row }">
                    <div
                        v-if="
                            row.manualFormData &&
                            Object.keys(row.manualFormData).length > 0
                        "
                        class="space-y-1 text-xs text-gray-600"
                    >
                        <div
                            v-for="(value, key) in row.manualFormData"
                            :key="key"
                        >
...
const filterStatus = ref<"pending" | "verified" | "rejected" | "all">(
    "pending",
);
...
async function handleApprove(row: StudentVerification) {
    try {
        await api.userAdmin.reviewStudentVerification(row.userID, {
            approved: true,
        });
...
async function handleReject() {
    ...
    try {
        await api.userAdmin.reviewStudentVerification(currentRow.value.userID, {
            approved: false,
