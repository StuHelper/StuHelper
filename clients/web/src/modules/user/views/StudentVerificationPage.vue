<template>
  <div class="max-w-[600px] mx-auto p-6 animate-fade-in max-sm:p-4">
    ...
    >
      ...
      <div v-if="profile.schoolID" class="text-sm text-text-secondary">
        <div class="flex justify-between">
          <span class="text-text-muted">{{ t('user.verification.student.selectSchool') }}</span>
    ...
    >
      ...
      <div class="mb-5">
        <label class="block text-sm font-semibold text-text-primary mb-2" for="student-school">
        ...
        >
          <option value="" disabled>{{ t('user.verification.student.selectSchool') }}</option>
          <option
      ...
      <template v-if="selectedSchool">
        <!-- Consent text -->
        <div v-if="selectedSchool.consentText" class="mb-5 p-3 bg-primary/5 border border-primary/20 rounded-lg">
          <p class="text-xs text-text-secondary m-0 whitespace-pre-line">
        ...
        <template v-if="selectedSchool.verificationMethod === 'ldap'">
          <div class="mb-4">
            <label class="block text-sm font-semibold text-text-primary mb-2" for="student-id">
            ...
            <input
              ...
              :placeholder="t('user.verification.student.studentId')"
            />
          </div>

          <div class="mb-5">
            <label class="block text-sm font-semibold text-text-primary mb-2" for="student-password">
            ...
            <input
              ...
              type="password"
              class="w-full px-3 py-2.5 bg-transparent border border-border rounded-lg text-sm text-text-primary placeholder-text-muted outline-none transition-all duration-fast focus:border-primary"
              :placeholder="t('user.verification.student.password')"
            />
          </div>
        </template>
        ...
        >
          <div class="flex items-center gap-2 text-sm text-text-muted">
            ...
            <span>{{ t('user.verification.student.verifyPending') }}</span>
          </div>
        </div>
...
import { GraduationCap, ShieldAlert, Clock, XCircle, ArrowLeft } from 'lucide-vue-next'
...
async function handleSubmit() {
  ...
  try {
    ...
    toast.error(err instanceof Error ? err.message : t('user.verification.student.verifyFailed'))
  } finally {
