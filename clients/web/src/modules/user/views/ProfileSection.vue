      ...
      <div class="border-0 rounded-lg p-4 transition-all duration-fast">
        <div class="flex items-center gap-3 mb-3">
          ...
          >
            <Phone
              class="size-5"
              :class="phoneBound ? 'text-green-500' : 'text-text-muted'"
            />
          </div>
          <div class="min-w-0">
            <p class="text-sm font-semibold text-text-primary m-0">
            ...
            <span
              class="inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium"
              :class="phoneBound ? 'bg-green-500/10 text-green-600' : 'bg-bg-base text-text-muted'"
            >
              {{ phoneBound ? t('user.verification.phone.bound') : t('user.verification.phone.unbound') }}
            </span>
          </div>
        </div>
        <router-link
  ...
  if (authStore.isAuthenticated) {
    void verificationStore.fetchStatus().catch((error) => {
      if (import.meta.env.DEV) {
        console.warn('[ProfileSection] failed to fetch verification status', error)
      }
    })
  }
})
