...
const status = ref<'published' | 'hidden' | 'deleted' | 'all'>('all')
...
const handleUpdate = async (id: string, action: 'hide' | 'restore' | 'delete') => {
  try {
...
const handleBatch = async (action: BatchUpdateParams['action']) => {
  try {
    await api.admin.batchUpdateReviews({ ids: selected.value, action })
