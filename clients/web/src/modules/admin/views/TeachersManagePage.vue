...
import { buildTeachersQuery } from '@/utils/filterQueryBuilders'
...
const { data: teachersData, loading, execute: fetchTeachers } = useAsyncData(async () => {
  const res = await api.admin.getTeachers(buildTeachersQuery({
    page: page.value,
    pageSize: pageSize.value,
    searchQuery: searchQuery.value,
    filterDeptID: filterDeptID.value,
  }))
  const list = res.data?.data?.list || []
  return {
    teachers: list.map((t): AdminTeacher => ({
      ...t,
