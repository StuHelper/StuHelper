...
import { buildSensitiveWordsQuery } from '@/utils/filterQueryBuilders'
...
const { data: wordsData, loading, execute: fetchWords } = useAsyncData(async () => {
  const res = await api.admin.getSensitiveWords(buildSensitiveWordsQuery({
    page: page.value,
...
const categories = computed(() => [...new Set(words.value.map(w => w.category).filter((c): c is string => !!c))].sort())
...
async function handleSave() {
  ...
  try {
    if (editing.value) {
      await api.admin.updateSensitiveWord(editing.value.id, {
      ...
      await api.admin.createSensitiveWord({
        ...
        level: formLevel.value as 'block' | 'warn' | 'review'
      })
...
async function toggleActive(w: AdminSensitiveWord) {
  try {
    await api.admin.updateSensitiveWord(w.id, { isActive: !w.isActive })
    toast.success(t('admin.sensitiveWords.updateSuccess'))
    fetchWords()
  } catch {
    toast.error(t('admin.sensitiveWords.saveFailed'))
...
async function handleDelete(w: AdminSensitiveWord) {
  ...
  try {
    await api.admin.deleteSensitiveWord(w.id)
