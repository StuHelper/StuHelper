...
vi.mock("@/components/common/ThemeSwitcher.vue", () => ({
    default: {
        name: "ThemeSwitcher",
        template: '<div data-test="theme-switcher" />',
    },
}));
        ...
        global: {
            ...
            stubs: {
                RouterLink: {
                    props: ["to"],
                    template:
                        '<a data-test="router-link" :data-to="to"><slot /></a>',
                },
            },
        },
    });
...
describe("AppHeader", () => {
    ...
    it("does not mount authenticated header actions while cached auth is unresolved", () => {
        ...
        expect(wrapper.find('[data-test="notification-bell"]').exists()).toBe(
            false,
        );
        expect(wrapper.find('[data-test="app-user-menu"]').exists()).toBe(
            false,
        );
    ...
    it("shows the login entry after session bootstrap resolves anonymous", () => {
        ...
        expect(wrapper.find('[data-test="locale-switcher"]').exists()).toBe(
            true,
        );
        expect(wrapper.find('[data-test="theme-switcher"]').exists()).toBe(
            true,
        );
        expect(wrapper.find('[data-test="notification-bell"]').exists()).toBe(
            false,
    ...
    it("keeps normal site navigation on the login route", () => {
        ...
        expect(wrapper.text()).not.toContain(
            "routes.openPlatformDeveloperApps",
        );
    ...
    it("shows the header course search only on the course hub", () => {
        ...
        expect(
            courseHubWrapper.find('[data-test="inline-search"]').exists(),
        ).toBe(true);
