<template>
    <header
        ...
        :class="
            isScrolled
                ? 'border-white/15 dark:border-white/8 shadow-sm'
                : 'border-transparent'
        "
    >
        <div
            class="mx-auto flex h-[var(--navbar-height)] max-w-[var(--max-width)] items-center gap-5 px-6 max-md:px-4 max-md:gap-2"
        >
            <router-link
                :to="logoRoute"
            ...
            >
                <span
                    class="grid size-9 place-items-center rounded-lg bg-gradient-to-br from-primary to-accent text-sm font-extrabold text-white shadow-glow-sm"
                >
                    S
                </span>
                <span class="flex min-w-0 flex-col leading-none">
                    ...
                    >
                    <span
                        class="mt-1 hidden text-[11px] font-medium text-text-muted xl:block"
                        >{{ brandTagline }}</span
                    >
                </span>
            </router-link>
            ...
            <div
                v-if="showCourseSearch"
                class="w-[min(40vw,520px)] min-w-[240px] shrink max-tablet:hidden"
            >
                <InlineSearch />
            </div>
        ...
        <div
            ...
            :style="{
                top: showCourseSearch
                    ? 'calc(var(--navbar-height) + 56px)'
                    : 'calc(var(--navbar-height) + 8px)',
            }"
        >
            <nav
...
const showWriteReview = computed(
    () =>
        route.path.startsWith("/courses") || route.path.startsWith("/teachers"),
);
const showAuthenticatedActions = computed(
    () => authStore.bootstrapCompleted && authStore.isAuthenticated,
);
const showNotificationBell = computed(() => showAuthenticatedActions.value);
