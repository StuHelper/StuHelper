...
async function mockSSOAuthPage(page: Page, title = "Mock SSO") {
...
test.describe("Auth Flow", () => {
        ...
        await page.route("**/api/v1/course/review/drafts**", (route) => {
            ...
            return route.fulfill({
                ...
                body: JSON.stringify({
                    ...
                    error: {
                        code: "E0050000",
                        message: "unexpected draft access",
                    },
                }),
            });
        });
    ...
    test("authenticated user can force SSO reauthentication from login route", async ({
        ...
        await page.route("**/api/v1/auth/login*", (route) => {
            ...
            return route.fulfill({
                ...
                body: JSON.stringify({
                    success: true,
                    data: {
                        url: "https://sso.stuhelper.com/login/oauth/authorize?client_id=stuhelper-web&state=reauth-state",
                        state: "reauth-state",
                    },
                }),
            });
        ...
        await Promise.all([
            page.waitForURL(isSSOAuthorizeURL),
            page
                .getByRole("button", {
                    name: /Continue with unified sign-in|使用统一身份认证登录/,
                })
    ...
    test("protected developer app route starts SSO with its own return target", async ({
        page,
    }) => {
        await mockUnauthenticated(page);
        ...
        await page.route("**/api/v1/auth/login*", (route) => {
            ...
            return route.fulfill({
                ...
                body: JSON.stringify({
                    success: true,
                    data: {
                        url: "https://sso.stuhelper.com/login/oauth/authorize?client_id=stuhelper-web&state=developer-state",
        ...
        await page.route("**/api/v1/auth/login*", async (route) => {
            ...
            return route.fulfill({
                ...
                body: JSON.stringify({
                    success: true,
                    data: {
                        url: "https://sso.stuhelper.com/login/oauth/authorize?client_id=stuhelper-web&state=sso-state",
                        state: "sso-state",
                    },
                }),
            });
    ...
    test("user center shortcut redirects to the reviews tab", async ({
        page,
    }) => {
        await mockAuthenticated(page, VERIFIED_STUDENT);
