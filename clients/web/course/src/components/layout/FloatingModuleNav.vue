<template>
  ...
  >
    ...
    <div class="relative group">
      ...
      <span id="tooltip-active" role="tooltip" class="absolute right-full mr-2.5 top-1/2 -translate-y-1/2 px-2 py-1 text-xs font-medium text-text-primary bg-bg-glass-heavy backdrop-blur-lg rounded-md shadow-sm border border-white/15 dark:border-white/8 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-fast pointer-events-none">
        {{ activeTab.label }}
      </span>
    </div>
    ...
    >
      <template v-if="expanded">
        ...
        <span :id="`tooltip-tab-${tab.to}`" role="tooltip" class="absolute right-full mr-2.5 top-1/2 -translate-y-1/2 px-2 py-1 text-xs font-medium text-text-primary bg-bg-glass-heavy backdrop-blur-lg rounded-md shadow-sm border border-white/15 dark:border-white/8 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-fast pointer-events-none">
          {{ tab.label }}
        </span>
      </div>
      </template>
...
const tabDefs: ModuleTab[] = [
  { to: '/review', icon: markRaw(MessageSquare), labelKey: 'nav.review' },
  { to: '/teacher', icon: markRaw(GraduationCap), labelKey: 'nav.teacher' },
  { to: '/spoc', icon: markRaw(BookOpen), labelKey: 'nav.spoc' },
  { to: '/resource', icon: markRaw(FolderOpen), labelKey: 'nav.resource' },
]

const tabs = computed(() => tabDefs.map(d => ({
  to: d.to,
  icon: d.icon,
  label: t(d.labelKey),
})))
