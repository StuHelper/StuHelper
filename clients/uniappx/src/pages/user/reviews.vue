...
const {
  ...
  async fetchPage(page, pageSize) {
    const result = await api.user.getMyReviews(page, pageSize)
    return unwrapListData<components['schemas']['Review']>(result)
  },
  onError(error) {
    uni.showToast({
      title: error instanceof Error ? error.message : t('user.reviews.loadFailed'),
      icon: 'none',
    })
  },
})
...
<template>
  <scroll-view class="page" scroll-y>
    ...
    <view v-else class="list-wrap">
      ...
      <A11yButton v-if="hasMore" class="load-more" data-testid="uni-user-reviews-load-more" :disabled="loadingMore" @tap="loadMore">
