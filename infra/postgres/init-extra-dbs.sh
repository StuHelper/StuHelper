...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres <<-EOSQL
    ...
    CREATE ROLE stuhelper_backup LOGIN PASSWORD '${STUHELPER_BACKUP_DB_PASSWORD}';
    CREATE ROLE stuhelper_replication WITH LOGIN REPLICATION PASSWORD '${STUHELPER_REPLICATION_DB_PASSWORD}';
