...
zitadel_secret_tmp="$(mktemp)"
chmod 600 "${zitadel_secret_tmp}"
...
ZITADEL_OUTPUT="$(
  ...
  ZITADEL_SECRET_OUTPUT_FILE="${zitadel_secret_tmp}" \
...
ZITADEL_CLIENT_SECRET="$(awk -F= '/^ZITADEL_CLIENT_SECRET=/{print $2}' "${zitadel_secret_tmp}" | tail -n1)"
...
log "bootstrapped Zitadel project ${ZITADEL_PROJECT_ID} and OIDC client ${ZITADEL_CLIENT_ID}"
...
trap 'rm -f "${zitadel_secret_tmp}" "${runtime_env_tmp}" "${secret_env_tmp}"' EXIT
