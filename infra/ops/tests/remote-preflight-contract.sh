...
assert_contains "${PREFLIGHT_FILE}" 'install-baota-runtime-permission-guard\.sh" --verify-installed'
assert_contains "${PREFLIGHT_FILE}" 'GENERATED_OBSERVABILITY_CONFIG_OWNER_UID='
assert_contains "${PREFLIGHT_FILE}" 'ALERTMANAGER_CONFIG_GID='
assert_contains "${PREFLIGHT_FILE}" '^load_env_preserving NGINX_PUBLIC_INGRESS_CONFIG_FILE$'
assert_not_contains "${PREFLIGHT_FILE}" '^load_env$'
