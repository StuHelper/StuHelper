...
assert_contains "${NODE_DOCKERFILE}" '^ARG BRACE_EXPANSION_VERSION=5\.0\.9$'
assert_contains "${NODE_DOCKERFILE}" '^ARG BRACE_EXPANSION_TARBALL_SHA512=[A-Za-z0-9+/]+=*$'
assert_contains "${NODE_DOCKERFILE}" '^ARG IP_ADDRESS_VERSION=10\.3\.1$'
assert_contains "${NODE_DOCKERFILE}" '^ARG IP_ADDRESS_TARBALL_SHA512=[A-Za-z0-9+/]+=*$'
assert_contains "${NODE_DOCKERFILE}" 'sha512sum "\$\{npm_tarball\}"'
assert_contains "${NODE_DOCKERFILE}" 'sha512sum "\$\{brace_expansion_tarball\}"'
assert_contains "${NODE_DOCKERFILE}" 'sha512sum "\$\{ip_address_tarball\}"'
