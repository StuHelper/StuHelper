...
assert_contains "${TARGET_SCRIPT}" 'if \[\[ ! -s "\$\{vault_token_file\}" \]\]; then'
assert_contains "${TARGET_SCRIPT}" 'Never regress it back to the initialization root token'
...
assert_contains "${TARGET_SCRIPT}" 'warn "keep \$\{vault_init_file\} and \$\{vault_unseal_key_file\} root-only and backed up; use a scoped runtime token at \$\{vault_token_file\} for deployment"'
