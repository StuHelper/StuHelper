...
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_PROVIDER must be oracle'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_SCHOOL_CODE must be a 10-digit school code'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_HOST'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_SERVICE_NAME'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_USERNAME'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_READONLY_USERNAME'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_PASSWORD'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_TLS_MODE must be verify-full in production'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_TLS_CA_HOST_PATH'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_TLS_CA_FILE must be /external-student-source-tls/ca.crt in production'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_SCHEMA'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_TABLE'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_STUDENT_ID_COLUMN'
assert_contains "${READINESS_SCRIPT}" 'EXTERNAL_STUDENT_SOURCE_ORACLE_STUDENT_NAME_COLUMN'
assert_contains "${READINESS_SCRIPT}" 'buaa_external_student_source_ready'
...
assert_contains "${READINESS_SCRIPT}" 'NOT \(SELECT buaa_external_student_source_ready FROM input\)'
...
assert_contains "${READINESS_SCRIPT}" 'admission production readiness passed: schools=%s policies=%s bot_credential=%s buaa_student_source=%s'
assert_contains "${READINESS_SCRIPT}" 'external_oracle'
assert_contains "${READINESS_SCRIPT}" 'local_academic_table'
