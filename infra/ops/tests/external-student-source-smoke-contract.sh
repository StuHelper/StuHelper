...
assert_contains "${SCRIPT_FILE}" 'runtime identity did not match the explicitly configured existing account'
assert_contains "${SCRIPT_FILE}" 'leastPrivilegeGrantsVerified'
assert_contains "${SCRIPT_FILE}" 'preferred least-privilege grant shape'
assert_contains "${SCRIPT_FILE}" 'invalid external Oracle'
...
assert_contains "${PROD_ENV_EXAMPLE}" '^EXTERNAL_STUDENT_SOURCE_ORACLE_READONLY_USERNAME=REPLACE_WITH_EXISTING_EXTERNAL_STUDENT_SOURCE_ORACLE_USERNAME$'
