...
const logger = new Logger('stuhelperGroupCenter:auth')
...
export class AuthModule extends BaseModule {
   ...
   */
  private findRole(roleIdentifier: string): { role: ReturnType<typeof this.ctx.stuhelperGroupCenter.auth.getRoles>[0] | null, warning?: string } {
    const allRoles = this.ctx.stuhelperGroupCenter.auth.getRoles()
  ...
  private registerCommands(): void {
    ...
    this.registerCommand({
      ...
      .action(async ({ session }) => {
        const roles = this.ctx.stuhelperGroupCenter.auth.getRoles()
        if (roles.length === 0) {
          return '暂无可用角色'
        }
        ...
        for (const role of roles) {
          ...
          const memberCount = isBuiltin ? '-' : this.ctx.stuhelperGroupCenter.auth.getRoleMembers(role.id).length
    ...
    this.registerCommand({
      ...
      .action(async ({ session }, target) => {
        ...
        const userRoleIds = this.ctx.stuhelperGroupCenter.auth.getUserRoleIds(userId)
        const allRoles = this.ctx.stuhelperGroupCenter.auth.getRoles()
    ...
    this.registerCommand({
      ...
      .action(async ({ session }, target, roleIdentifier) => {
        ...
        try {
          await this.ctx.stuhelperGroupCenter.auth.assignRole(userId, role.id)
    ...
    this.registerCommand({
      ...
      .action(async ({ session }, target, roleIdentifier) => {
        ...
        try {
          await this.ctx.stuhelperGroupCenter.auth.revokeRole(userId, role.id)
