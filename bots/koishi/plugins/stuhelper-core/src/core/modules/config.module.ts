...
export class ConfigModule extends BaseModule {
  ...
  private registerCommands(): void {
    // 手动注册额外的权限节点
    this.ctx.stuhelperGroupCenter.auth.registerPermission('config.view', '查看配置', '查看所有配置和记录', this.meta.description)
    this.ctx.stuhelperGroupCenter.auth.registerPermission('config.blacklist', '黑名单管理', '管理黑名单（添加/移除）', this.meta.description)
    this.ctx.stuhelperGroupCenter.auth.registerPermission('config.warn', '警告管理', '管理警告记录（添加/移除）', this.meta.description)
    ...
    this.registerCommand({
      ...
      .action(async ({ session, options }, content) => {
        ...
        // 显示所有配置
        if (options.t) {
          if (!this.ctx.stuhelperGroupCenter.auth.check(session, 'config.view')) {
            return '你没有权限查看配置喵...'
          }
        ...
        // 黑名单管理
        if (options.b) {
          if (!this.ctx.stuhelperGroupCenter.auth.check(session, 'config.blacklist')) {
            return '你没有权限管理黑名单喵...'
          }
        ...
        // 警告管理
        if (options.w) {
          if (!this.ctx.stuhelperGroupCenter.auth.check(session, 'config.warn')) {
            return '你没有权限管理警告喵...'
          }
...
${formatMutes || '无记录'}`
  ...
  private async handleBlacklist(session: any, options: any, content: string): Promise<string> {
    ...
    if (options.a) {
      ...
      await this.ctx.stuhelperGroupCenter.pushMessage(session.bot, `[黑名单] 用户 ${options.a} 被加入黑名单`, 'blacklist')
    ...
    if (options.r) {
      ...
      await this.ctx.stuhelperGroupCenter.pushMessage(session.bot, `[黑名单] 用户 ${options.r} 被移出黑名单`, 'blacklist')
