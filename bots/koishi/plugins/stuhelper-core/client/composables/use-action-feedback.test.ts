...
test('useActionFeedback normalizes unknown causes with fallback text', () => {
  ...
  assert.equal(feedback.errorMessage({ message: ' backend failed ' }, '默认错误'), 'backend failed')
  assert.equal(feedback.errorMessage({ error: new Error('remote failed') }, '默认错误'), 'remote failed')
  assert.equal(feedback.errorMessage({ reason: 'policy denied' }, '默认错误'), 'policy denied')
  assert.equal(feedback.errorMessage({ code: 'E_TEST' }, '默认错误'), '默认错误')
})
