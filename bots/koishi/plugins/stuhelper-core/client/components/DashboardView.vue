  ...
  <div class="dashboard-container">
    ...
    <div class="hero-section">
      <div class="hero-content">
        <div class="hero-icon">
          <k-icon name="stuhelperGroupCenter:logo" />
        </div>
        <div class="hero-text">
          <h1>StuHelper 群管中心</h1>
          <div class="hero-meta">
...
const availableWidgets = [
  { type: 'NoticeCard', name: '公告卡片', description: '显示公告内容', icon: 'stuhelperGroupCenter:octicons.megaphone', span: 2, defaultDynamicProps: { content: 'notice' } },
  { type: 'TrendChartCard', name: '趋势图表', description: '显示命令使用趋势', icon: 'stuhelperGroupCenter:octicons.graph', span: 2, defaultDynamicProps: { data: 'chartData.trend', loading: 'chartLoading' } },
  { type: 'DistChartCard', name: '命令排行', description: '显示命令使用分布', icon: 'stuhelperGroupCenter:octicons.bar-chart', span: 1, defaultDynamicProps: { data: 'chartData.distribution', loading: 'chartLoading' } },
  { type: 'RankChartCard', name: '群聊排行', description: '显示群聊活跃排行', icon: 'stuhelperGroupCenter:octicons.people', span: 1, defaultProps: { type: 'guild', title: '群聊排行', icon: 'octicons.people' }, defaultDynamicProps: { data: 'chartData.guildRank', loading: 'chartLoading' } },
  { type: 'RankChartCard', name: '个人排行', description: '显示用户活跃排行', icon: 'stuhelperGroupCenter:octicons.person', span: 1, defaultProps: { type: 'user', title: '个人排行', icon: 'octicons.person' }, defaultDynamicProps: { data: 'chartData.userRank', loading: 'chartLoading' } },
  { type: 'VersionCard', name: '版本信息', description: '显示版本信息', icon: 'stuhelperGroupCenter:octicons.tag', span: 1, defaultDynamicProps: { stats: 'stats', versions: 'versions' } },
  { type: 'UpdatesCard', name: '最近更新', description: '显示最近的代码提交', icon: 'stuhelperGroupCenter:octicons.history', span: 2, defaultDynamicProps: { commits: 'commits', error: 'commitsError' } }
]
...
const loadNotice = async () => {
  try {
    ...
    const res = await fetch(`https://raw.githubusercontent.com/Camvanaa/koishi-plugin-stuhelperGroupCenter/dev/notice.md?t=${timestamp}`, { cache: 'no-store' })
...
const loadVersions = async () => {
  ...
  try {
    fetch(`https://raw.githubusercontent.com/Camvanaa/koishi-plugin-stuhelperGroupCenter/main/package.json?t=${timestamp}`, fetchOptions).then(res => res.json()).then(pkg => versions.main = pkg.version).catch(() => versions.main = 'Fail')
    fetch(`https://raw.githubusercontent.com/Camvanaa/koishi-plugin-stuhelperGroupCenter/dev/package.json?t=${timestamp}`, fetchOptions).then(res => res.json()).then(pkg => versions.dev = pkg.version).catch(() => versions.dev = 'Fail')
    fetch('https://registry.npmjs.org/koishi-plugin-stuhelperGroupCenter/latest', fetchOptions).then(res => res.json()).then(pkg => versions.npm = pkg.version).catch(() => versions.npm = 'Fail')
  } catch (e) {}
}
...
const loadCommits = async () => {
  try {
    commitsError.value = ''
    const res = await fetch('https://api.github.com/repos/Camvanaa/koishi-plugin-stuhelperGroupCenter/commits?sha=dev&per_page=5')
