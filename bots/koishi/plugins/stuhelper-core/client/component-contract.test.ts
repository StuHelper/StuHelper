...
test('RolesView keeps role operation failures visible', () => {
  ...
  assert.match(source, /const actionError = ref\(''\)/)
  assert.match(source, /const actionErrorTitle = ref\('操作失败'\)/)
  assert.match(source, /const importError = ref\(''\)/)
  assert.match(source, /const importErrorTitle = ref\('导入失败'\)/)
  ...
  assert.match(source, /function setActionError\(title: string, cause: unknown, fallback: string\): void/)
  assert.match(source, /function setImportError\(title: string, cause: unknown, fallback: string\): void/)
  assert.match(source, /message\.error\(`\$\{title\}: \$\{details\}`\)/)
