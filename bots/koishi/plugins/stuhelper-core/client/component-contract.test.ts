...
test('page API client uses typed Koishi send events without double casts', () => {
  ...
  assert.doesNotMatch(source, /saveGuardBinding/)
  assert.doesNotMatch(typesSource, /save-guard-binding/)
})
...
test('legacy console interactions preserve keyboard and modal accessibility contracts', () => {
  ...
  assert.match(chatSource, /role="menu"[\s\S]*?aria-label="消息操作"/)
...
test('SettingsView manages group-guard keyword rules through dedicated WebUI API', () => {
  const source = readClientFile('./components/SettingsView.vue')
  const apiSource = readClientFile('./api.ts')

  assert.match(apiSource, /export const keywordRulesApi = \{/)
  assert.match(apiSource, /stuhelperGroupCenter\/keyword-rules\/list/)
  assert.match(apiSource, /stuhelperGroupCenter\/keyword-rules\/upsert/)
  assert.match(apiSource, /stuhelperGroupCenter\/keyword-rules\/delete/)
  assert.match(source, /keywordRulesApi,/)
  assert.match(source, /keywordRules: KeywordRule\[\]/)
  assert.match(source, /keywordRulesApi\.list\(\)/)
  assert.match(source, /keywordRulesApi\.upsert\(buildKeywordRulePayload\(rule\)\)/)
  assert.match(source, /keywordRulesApi\.delete\(id\)/)
  assert.match(source, /saveKeywordRuleChanges\(\{/)
  assert.match(source, /assertSafeKeywordRegex\(pattern, 256\)/)
  assert.match(source, /delete payload\.keywordRules/)
  assert.match(source, /class="keyword-rule-list"/)
  assert.match(source, /class="keyword-rule-editor"/)
  assert.match(source, /class="keyword-rule-actions"/)
  assert.match(source, /class="mono-control"/)
  assert.match(source, /class="settings-select"/)
})
...
test('ConfigView keeps config operation failures visible', () => {
  ...
  assert.match(source, /<button type="button" class="btn btn-primary" @click="openCreateDialog">/)
...
test('SettingsView blocks default-form edits when global settings fail to load', () => {
  ...
  assert.match(source, /loadError\.value = ''\s*clearActionError\(\)\s*try \{/)
  assert.match(source, /const \[data, bindingData, adminData, groupGuardAIData, groupGuardData, groupGuardMessageData, keywordRulesData\] = await Promise\.all\(\[/)
  assert.match(source, /settingsApi\.get\(\),/)
  assert.match(source, /bindingSettingsApi\.get\(\),/)
  assert.match(source, /adminSettingsApi\.get\(\),/)
  assert.match(source, /groupGuardAISettingsApi\.get\(\),/)
  assert.match(source, /groupGuardBehaviorSettingsApi\.get\(\),/)
  assert.match(source, /groupGuardMessageSettingsApi\.get\(\),/)
  assert.match(source, /keywordRulesApi\.list\(\),/)
  assert.match(source, /if \(requestSeq !== loadRequestSeq\) return false/)
...
test('SettingsView manages group guard AI review through WebUI runtime settings without plaintext key echo', () => {
  const source = readClientFile('./components/SettingsView.vue')
  const apiSource = readClientFile('./api.ts')

  assert.match(apiSource, /export interface GroupGuardAISettings \{[\s\S]*?apiKeyConfigured: boolean[\s\S]*?apiKeyMasked: string/)
  assert.match(apiSource, /export interface GroupGuardAISettingsUpdate \{[\s\S]*?newApiKey\?: string[\s\S]*?clearApiKey\?: boolean/)
  assert.match(apiSource, /stuhelperGroupCenter\/group-guard-ai-settings\/get/)
  assert.match(apiSource, /stuhelperGroupCenter\/group-guard-ai-settings\/update/)
  assert.match(apiSource, /stuhelperGroupCenter\/group-guard-ai-settings\/reset/)
  assert.match(source, /groupGuardAISettingsApi/)
  assert.match(source, /type GroupGuardAISettings/)
  assert.match(source, /type GroupGuardAISettingsUpdate/)
  assert.match(source, /groupGuardAI: GroupGuardAISettings/)
  assert.match(source, /settings\.groupGuardAI\.enabled/)
  assert.match(source, /settings\.groupGuardAI\.endpoint/)
  assert.match(source, /settings\.groupGuardAI\.model/)
  assert.match(source, /const groupGuardAIApiKeyDraft = ref\(''\)/)
  assert.match(source, /const clearGroupGuardAIApiKey = ref\(false\)/)
  assert.match(source, /function normalizeGroupGuardAISettings\(data: unknown\): GroupGuardAISettings/)
  assert.match(source, /function applyGroupGuardAIApiKeyMetadata\(data: unknown, clearIntent = true\)/)
  assert.match(source, /function stripGroupGuardAIApiKeyMetadata\(model: SettingsModel\)/)
  assert.match(source, /delete payload\.groupGuardAI/)
  assert.match(source, /delete ai\.apiKey/)
  assert.match(source, /delete ai\.apiKeyConfigured/)
  assert.match(source, /delete ai\.apiKeyMasked/)
  assert.match(source, /function buildGroupGuardAISettingsPayload\([\s\S]*?\): GroupGuardAISettingsUpdate/)
  assert.match(source, /payload\.clearApiKey = true/)
  assert.match(source, /payload\.newApiKey = keyIntent\.newApiKey/)
  assert.match(source, /await groupGuardAISettingsApi\.update\([\s\S]*?buildGroupGuardAISettingsPayload\(plan\.model, plan\.groupGuardAIKey\)/)
  assert.doesNotMatch(source, /groupGuardAISettingsApi\.update\(settings\.value\.groupGuardAI\)/)
})

test('SettingsView manages group guard messages through WebUI runtime settings', () => {
  const source = readClientFile('./components/SettingsView.vue')
  const apiSource = readClientFile('./api.ts')

  assert.match(apiSource, /export interface GroupGuardMessageSettings \{\s*messages: Record<string, string>/)
  assert.match(apiSource, /stuhelperGroupCenter\/group-guard-message-settings\/get/)
  assert.match(apiSource, /stuhelperGroupCenter\/group-guard-message-settings\/update/)
  assert.match(apiSource, /stuhelperGroupCenter\/group-guard-message-settings\/reset/)
  assert.match(source, /groupGuardMessageSettingsApi/)
  assert.match(source, /type GroupGuardMessageSettings/)
  assert.match(source, /groupGuardMessages: GroupGuardMessageSettings/)
  assert.match(source, /activeSection === 'groupGuardMessages'/)
  assert.match(source, /v-for="field in groupGuardMessageFields"/)
  assert.match(source, /v-model="settings\.groupGuardMessages\.messages\[field\.key\]"/)
  assert.match(source, /function normalizeGroupGuardMessageSettings\(data: unknown\): GroupGuardMessageSettings/)
  assert.match(source, /delete payload\.groupGuardMessages/)
  assert.match(source, /function buildGroupGuardMessageSettingsPayload\(model: SettingsModel\): GroupGuardMessageSettings/)
  assert.match(source, /await groupGuardMessageSettingsApi\.update\([\s\S]*?buildGroupGuardMessageSettingsPayload\(plan\.model\)/)
...
test('SettingsView keeps save failures visible and prevents duplicate submissions', () => {
  ...
  assert.match(source, /<strong>\{\{ actionErrorTitle \}\}<\/strong>/)
  assert.match(source, /<span>\{\{ actionError \}\}<\/span>/)
  assert.match(source, /@click="clearSaveError"/)
  assert.match(source, /aria-label="本次保存结果"/)
  assert.match(source, /@click="reloadSettingsAfterSaveFailure"/)
  assert.match(source, /:disabled="!settingsLoaded \|\| saving"/)
  assert.match(source, /<button type="button" class="save-bar-btn secondary" :disabled="saving" @click="resetChanges">放弃更改<\/button>/)
  assert.match(source, /<button type="button" class="save-bar-btn primary" :disabled="saving" @click="saveSettings">/)
  ...
  assert.match(source, /saving\.value = true\s*clearSaveError\(\)\s*try \{/)
  assert.match(source, /await runSettingsSaveSteps\(\[/)
  assert.match(source, /commitCoreSettingsBaseline\(plan\.model\)/)
  assert.match(source, /commitConfirmedSettingsSlice\('binding', plan\.model\.binding, persisted\)/)
  assert.match(source, /saveStepResults\.value = results\.map\(\(result\) => \(\{ \.\.\.result \}\)\)/)
  assert.match(source, /setActionError\(title, cause, '保存设置失败'\)/)
  assert.match(source, /title: '重新加载服务器设置'[\s\S]*?await loadSettings\(\)/)
  assert.doesNotMatch(source, /const actionError = ref\(''\)/)
  assert.doesNotMatch(source, /const actionErrorTitle = ref\('操作失败'\)/)
  assert.doesNotMatch(source, /function setActionError\(title: string, cause: unknown, fallback: string\): void/)
  assert.doesNotMatch(source, /function clearActionError\(\): void/)
...
test('AdmissionView exposes admission policy navigation without editable target bindings', () => {
  const source = readClientFile('./components/AdmissionView.vue')
  ...
  assert.match(source, />\s*查看同步绑定\s*<\/el-button>/)
  assert.match(source, /title="同步目标群"/)
  ...
  assert.doesNotMatch(source, /管理群绑定/)
...
test('ConfigCenterView keeps governance save failures separate from load failures', () => {
  ...
  assert.match(viewSource, /title="同步绑定"/)
  assert.match(viewSource, /目标认证群请在 Admin 后台入群认证策略中修改/)
  assert.doesNotMatch(viewSource, /保存绑定/)
  assert.doesNotMatch(composableSource, /validateBindingForm|saveGuardBinding|submitBinding|bindingForm/)
...
test('RolesView keeps role permission load failures visible and retryable', () => {
  ...
  assert.match(source, /<button type="button" class="secondary-btn" @click="fetchData">重试<\/button>/)
...
test('RolesView prevents duplicate role lifecycle mutations', () => {
  ...
  assert.match(source, /<button type="button" class="clone-btn" :disabled="Boolean\(roleOperation\)" @click="cloneRole">/)
  assert.match(source, /\{\{ roleOperation === 'clone' \? '克隆中…' : '克隆角色' \}\}/)
  assert.match(source, /<button type="button" class="danger-btn" :disabled="Boolean\(roleOperation\)" @click="deleteRole">/)
...
test('RolesView prevents duplicate role save submissions', () => {
  ...
  assert.match(source, /<button type="button" class="reset-btn" :disabled="savingChanges" @click="resetChanges">重置<\/button>/)
  assert.match(source, /<button type="button" class="save-btn" :disabled="savingChanges" @click="saveChanges">/)
...
test('RolesView prevents duplicate role member mutations', () => {
  ...
  assert.match(source, /<button type="button" class="primary-btn" :disabled="memberLoading \|\| addingMember \|\| !newMemberId\.trim\(\)" @click\.stop="handleAddMember">/)
  assert.match(source, /\{\{ addingMember \? '添加中…' : '添加成员' \}\}/)
  assert.match(source, /<button type="button" class="secondary-btn" :disabled="memberLoading \|\| addingMember" @click\.stop="openImportDialog">导入成员<\/button>/)
