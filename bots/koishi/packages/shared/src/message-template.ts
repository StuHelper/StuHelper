...
export const DEFAULT_GROUP_GUARD_MESSAGES: StuhelperGroupGuardMessageConfig = Object.freeze({
  ...
  admissionTimeCodeReminder: [
    ...
    '请按群公告中的入群验证码规则计算四位验证码；验证码以用户发送验证码消息时的北京时间(UTC+8)为准，允许前后 {toleranceSeconds} 秒误差。',
    '超时未验证将自动移出群聊。',
  ].join('\n'),
  ...
  admissionSkipUnmuteFailed: [
    '{at} ({qqID}) 已跳过本群入群认证，但自动解除禁言失败，请手动解除禁言。',
    '错误：{error}',
    '此操作只在本群生效，不代表 StuHelper 学生认证已通过。',
  ].join('\n'),
  ...
  admissionConsoleReleaseBlacklistNotFound: 'QQ {qqID} 在本群没有活动入群拉黑记录。',
  ...
  admissionConsoleErrorNotFound: '平台侧未找到 QQ {qqID} 在本群的入群认证会话；记录可能已结束或已被其它任务清理，请刷新页面后确认。',
